...
bool Address::Lookup(std::vector<Address::ptr>& result, const std::string& host,
    ...
    if (error) {
        ALOTZ_LOG_ERROR(g_logger) << "Address::Lookup getaddress(" << host << ", "
...
bool Address::GetInterfaceAddress(std::multimap<std::string,
    std::pair<Address::ptr, uint32_t>>& result, int family) {
        ...
        if (getifaddrs(&results) != 0) {
            ALOTZ_LOG_ERROR(g_logger) << "Address:GetInterfaceAddress getifaddrs "
        ...
        try {
            ...
            ALOTZ_LOG_ERROR(g_logger) << "Address::GetInterfaceAddress exception";
            freeifaddrs(results);
            return false;
        }
        freeifaddrs(results);
...
IPAddress::ptr IPAddress::Create(const char* address, uint16_t port) {
    ...
    if (error) {
        ALOTZ_LOG_ERROR(g_logger) << "IPAddress::Create(" << address
            << ", " << port << ") error=" << error
...
IPv4Address::ptr IPv4Address::Create(const char* address, uint16_t port) {
    ...
    if (result <= 0) {
        ALOTZ_LOG_ERROR(g_logger) << "IPv4Address::Create(" << address << ", "
...
IPAddress::ptr IPv4Address::subnetMask(uint32_t prefix_len) {
    ...
    return IPv4Address::ptr(new IPv4Address);
}
...
IPv6Address::ptr IPv6Address::Create(const char* address, uint16_t port) {
    ...
    if (result <= 0) {
        ALOTZ_LOG_ERROR(g_logger) << "IPv6Address::Create(" << address << ", "
            << port << ") rt=" << result << " errno=" << errno
            << " errstr=" << strerror(errno);
        return nullptr;
    }
