...
static alotz::ConfigVar<std::map<std::string, std::map<std::string, std::string> > >::ptr g_mysql_dbs
    = alotz::Config::Lookup("mysql.dbs", std::map<std::string, std::map<std::string, std::string> >()
            , "mysql dbs");
...
static MYSQL* mysql_init(std::map<std::string, std::string>& params,
    ...
    if(mysql == nullptr) {
        ALOTZ_LOG_ERROR(g_logger) << "mysql_init error";
        return nullptr;
    }
    ...
    int port = alotz::GetParamValue(params, "port", 0);
    std::string host = alotz::GetParamValue<std::string>(params, "host");
    std::string user = alotz::GetParamValue<std::string>(params, "user");
    std::string passwd = alotz::GetParamValue<std::string>(params, "password");
    std::string dbname = alotz::GetParamValue<std::string>(params, "database");

    if(mysql_real_connect(mysql, host.c_str(), user.c_str(), passwd.c_str()
                          ,dbname.c_str(), port, NULL, 0) == nullptr) {
        ALOTZ_LOG_ERROR(g_logger) << "mysql_real_connect(" << host
                                  << ", " << port << ", " << dbname
...
bool MySQL::connect() {
    ...
    m_hasError = false;
    m_poolSize = alotz::GetParamValue(m_params, "pool", 5);
    m_mysql.reset(m, mysql_close);
    return true;
}

alotz::IStmt::ptr MySQL::prepare(const std::string& sql) {
...
int MySQL::execute(const char* format, va_list ap) {
    m_cmd = alotz::StringUtil::Formatv(format, ap);
    int r = ::mysql_query(m_mysql.get(), m_cmd.c_str());
    if(r) {
        ALOTZ_LOG_ERROR(g_logger) << "cmd=" << cmd()
            << ", error: " << getErrStr();
        m_hasError = true;
    } else {
        m_hasError = false;
...
std::shared_ptr<MySQL> MySQL::getMySQL() {
    return MySQL::ptr(this, alotz::nop<MySQL>);
}
...
static MYSQL_RES* my_mysql_query(MYSQL* mysql, const char* sql) {
    if(mysql == nullptr) {
        ALOTZ_LOG_ERROR(g_logger) << "mysql_query mysql is null";
        return nullptr;
    }

    if(sql == nullptr) {
        ALOTZ_LOG_ERROR(g_logger) << "mysql_query sql is null";
        return nullptr;
    }

    if(::mysql_query(mysql, sql)) {
        ALOTZ_LOG_ERROR(g_logger) << "mysql_query(" << sql << ") error:"
            << mysql_error(mysql);
        return nullptr;
    }
    ...
    if(res == nullptr) {
        ALOTZ_LOG_ERROR(g_logger) << "mysql_store_result() error:"
            << mysql_error(mysql);
    }
    return res;
}
...
MySQLStmt::ptr MySQLStmt::Create(MySQL::ptr db, const std::string& stmt) {
    ...
    if(mysql_stmt_prepare(st, stmt.c_str(), stmt.size())) {
        ALOTZ_LOG_ERROR(g_logger) << "stmt=" << stmt
...
int MySQLStmt::bindTime(int idx, const time_t& value) {
    ...
    //return 0;
    return bindString(idx, alotz::Time2Str(value));
}
...
int64_t MySQLRes::getInt64(int idx) {
    return alotz::TypeUtil::Atoi(m_cur[idx]);
}
...
double MySQLRes::getDouble(int idx) {
    return alotz::TypeUtil::Atof(m_cur[idx]);
}
...
time_t MySQLRes::getTime(int idx) {
    ...
    return alotz::Str2Time(m_cur[idx]);
}
...
int MySQLTransaction::execute(const char* format, va_list ap) {
    if(m_isFinished) {
        ALOTZ_LOG_ERROR(g_logger) << "transaction is finished, format=" << format;
        return -1;
    }
...
int MySQLTransaction::execute(const std::string& sql) {
    if(m_isFinished) {
        ALOTZ_LOG_ERROR(g_logger) << "transaction is finished, sql=" << sql;
        return -1;
    }
    ...
    if(it != m_conns.end()) {
        if(!it->second.empty()) {
            ...
            if(rt->ping()) {
                ...
                ALOTZ_LOG_WARN(g_logger) << "reconnect " << name << " fail";
                return nullptr;
            }
        }
    }
...
int MySQLManager::execute(const std::string& name, const char* format, va_list ap) {
    auto conn = get(name);
    if(!conn) {
        ALOTZ_LOG_ERROR(g_logger) << "MySQLManager::execute, get(" << name
            << ") fail, format=" << format;
        return -1;
    }
...
ISQLData::ptr MySQLManager::query(const std::string& name, const char* format, va_list ap) {
    auto conn = get(name);
    if(!conn) {
        ALOTZ_LOG_ERROR(g_logger) << "MySQLManager::query, get(" << name
            << ") fail, format=" << format;
        return nullptr;
    }
...
MySQLTransaction::ptr MySQLManager::openTransaction(const std::string& name, bool auto_commit) {
    auto conn = get(name);
    if(!conn) {
        ALOTZ_LOG_ERROR(g_logger) << "MySQLManager::openTransaction, get(" << name
            << ") fail";
        return nullptr;
    }
