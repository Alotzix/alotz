...
HttpConnection::~HttpConnection() {
    ALOTZ_LOG_DEBUG(g_logger) << "HttpConnection::~HttpConnection";
}
...
HttpResponse::ptr HttpConnection::recvResponse() {
    ...
    if (client_parser.chunked) {
        ...
        do {
            ...
            ALOTZ_LOG_INFO(g_logger) << "content_len= " << client_parser.content_len;
...
HttpConnection::ptr HttpConnectionPool::getConnection() {
    ...
    if(!ptr) {
        ...
        if(!addr) {
            ALOTZ_LOG_ERROR(g_logger) << "get addr fail: " << m_host;
            return nullptr;
        }
        addr->setPort(m_port);
        Socket::ptr sock = Socket::CreateTCP(addr);
        if(!sock) {
            ALOTZ_LOG_ERROR(g_logger) << "create sock fail: " << *addr;
            return nullptr;
        }
        if(!sock->connect(addr)) {
            ALOTZ_LOG_ERROR(g_logger) << "sock connect fail: " << *addr;
            return nullptr;
        }
...
void HttpConnectionPool::ReleasePtr(HttpConnection* ptr, HttpConnectionPool* pool) {
    ++ptr->m_request;
    if(!ptr->isConnected()
            || ((ptr->m_createTime + pool->m_maxAliveTime) >= alotz::GetCurrentMS())
